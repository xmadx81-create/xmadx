...
async function _aiProcessChat(input, _detections) {
  ...
  if (/일하기\s*싫|퇴근하고\s*싶|집에?\s*가고\s*싶|일\s*안\s*하고\s*싶|놀고\s*싶|쉬고\s*싶/.test(t)) {
    return { reply: _pick([
      _say('그 마음 충분히 이해해요 😊\n남은 것만 빠르게 정리하고 쉬어요!', '이해해~ 남은 거만 빨리 끝내자!'),
      _say('저도 쉬고 싶... (AI라 못 쉬지만 ㅋ)\n오늘 할 것만 정리해드릴까요?', '나도 쉬고 싶다~ ㅋ 오늘 할 거만 정리해줄까?'),
    ]), suggests: ['오늘 마무리', '알아서해줘'] };
  }
  ...
  if (/^나\s*(배고프|배고파|짜증나|짜증|화나|화남|행복해|행복|기뻐|슬퍼|슬프|우울|우울해|신나|즐거|심심해)/.test(t)) {
    if (/배고프|배고파/.test(t)) return { reply: _say('배고프시군요! 맛있는 거 드세요 🍜', '배고프구나~ 맛있는 거 먹어! 🍜'), suggests: ['점심 추천'] };
    if (/짜증|화나|화남/.test(t)) return { reply: _say('화가 나셨군요... 얘기해주시면 들을게요.', '화났구나... 얘기해봐, 들어줄게.'), suggests: ['농담 해줘', '응원해줘'] };
    if (/행복|기뻐|신나|즐거/.test(t)) return { reply: _say('좋은 소식이 있나 봐요! 저도 기분 좋아요 😊', '오 좋겠다! 나도 기분 좋아~ 😊'), suggests: ['오늘 브리핑'] };
    if (/슬프|슬퍼|우울/.test(t)) return { reply: _say('마음이 안 좋으신 거예요? 옆에 있을게요 😢', '힘들어? 옆에 있을게 😢'), suggests: ['농담 해줘', '응원해줘'] };
    if (/심심해/.test(t)) return { reply: _say('심심하시군요! 제가 재밌게 해드릴까요? 😄', '심심해? 재밌는 거 해줄까? 😄'), suggests: ['농담 해줘', '명언'] };
  }
  ...
  if (t.length >= 3 && t.length <= 15 && /[다네군요지]$/.test(t) && !/해줘|할래|하자|할까|해주|시켜/.test(t)) {
    return { reply: _pick([
      _say('네, 그렇죠! 😊 더 얘기해주세요!', '응응~ 더 말해봐!'),
      _say('그러시군요! 뭐가 필요하시면 말씀해주세요!', '그렇구나~ 필요한 거 있으면 말해!'),
      _say('맞아요! 더 궁금한 거 있으세요?', '맞아~ 더 궁금한 거 있어?'),
    ]), suggests: ['오늘 브리핑'] };
  }
