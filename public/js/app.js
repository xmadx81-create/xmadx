...
async function _aiProcessChat(input, _detections) {
  ...
  // --- 도움말 / 사용법 안내 ---
  if (/도움말|뭐\s*할\s*수|사용법\s*안내|사용법|어떻게\s*써|뭐\s*할\s*줄|알려줘\s*기능/.test(t)) {
    return { reply: '📱 AI 비서 사용법 안내\n━━━━━━━━━━━━━━\n\n' +
      '아래 카테고리를 눌러 자세히 알아보세요!\n\n' +
      '📝 업무일지 — 말로 자동 작성\n✅ 할 일 — 추가·확인·관리\n📅 일정 — 등록·조회\n⏰ 출퇴근 — 출근·퇴근 체크\n📊 분석 — 브리핑·통계·예측\n🔍 검색 — 통합 검색·웹 검색\n💜 기타 — 기억·추천·감정 대화',
      suggests: ['업무일지 사용법', '할일 사용법', '일정 사용법', '전체 명령어'] };
  }
  if (/업무일지\s*사용법/.test(t)) {
    return { reply: '📝 업무일지 작성 3가지 방법\n━━━━━━━━━━━━━━\n\n' +
      '1️⃣ 자연어 자동 작성 (추천!)\n"오늘 강남에서 미팅 다녀왔어 계약 진행중"\n→ AI가 자동 분석 → 미리보기 → 저장/수정\n\n' +
      '2️⃣ 보고서 마법사\n"보고서 마법사" 입력\n→ 4단계 질문에 답하면 자동 완성\n\n' +
      '3️⃣ 직접 작성\n"직접 작성" 입력 → 보고서 폼 열기\n\n' +
      '💡 "보고서 확인" → 오늘 작성한 일지 조회\n💡 "업무일지 보기" → 일지 목록 이동',
      suggests: ['보고서 마법사', '직접 작성', '보고서 확인', '사용법 안내'] };
  }
  if (/할일\s*사용법/.test(t)) {
    return { reply: '✅ 할 일 관리\n━━━━━━━━━━━━━━\n\n' +
      '추가: "회의 준비 해야 돼" → 자동 인식\n추가: "할 일 추가 보고서 제출" → 직접 추가\n마법사: "할 일 마법사" → 기한까지 설정\n확인: "할 일 확인" → 미완료 목록\n이동: "할 일 관리 열기" → 페이지 이동',
      suggests: ['할 일 확인', '할 일 마법사', '사용법 안내'] };
  }
  if (/일정\s*사용법/.test(t)) {
    return { reply: '📅 일정 관리\n━━━━━━━━━━━━━━\n\n' +
      '자연어 등록: "3시에 미팅 있어" → 자동 인식\n마법사: "일정 등록할래" → 단계별 등록\n확인: "오늘 일정" → 오늘 일정 조회\n내일: "내일 일정" → 내일 일정 조회\n이동: "캘린더 열어" → 캘린더 페이지',
      suggests: ['오늘 일정', '일정 등록할래', '사용법 안내'] };
  }
  if (/전체\s*명령어/.test(t)) {
    return { reply: '✨ 전체 명령어 목록\n━━━━━━━━━━━━━━\n\n📱 이동 — "캘린더 열어", "할 일 보여줘"\n📅 일정 — "3시에 미팅 있어", "내일 일정"\n✅ 할 일 — "회의록 추가해", "할 일 마법사"\n📝 보고서 — "보고서 마법사", "직접 작성"\n⏰ 출퇴근 — "출근해", "퇴근해"\n⏰ 알림 — "30분 뒤 알려줘", "3시에 알려줘"\n📊 분석 — "오늘 브리핑", "주간 리포트"\n📋 일지 — "오늘 뭐했지", "생산성 트렌드"\n🔮 예측 — "오늘 예측", "마감 위험"\n🧠 추천 — "추천해줘", "우선순위"\n💜 기억 — "나는 ENFP야", "기억해"\n🔍 검색 — "구글 OOO", "웹검색 OOO"\n🎤 음성 — 마이크 버튼으로 음성 대화\n💡 "아까 그거", "다시 해줘" 참조 가능', suggests: ['오늘 브리핑', '보고서 마법사', '사용법 안내'] };
  }
  if (/기능/.test(t) && !/삭제|수정|이상|안\s*돼|왜/.test(t)) {
    return { reply: '이 앱에서 할 수 있는 것들이에요!\n\n📝 업무일지 작성·관리\n✅ 할 일 관리\n📅 일정·캘린더\n⏰ 출퇴근 체크\n💬 팀 게시판\n📊 AI 인사이트·분석\n📋 업무 매뉴얼\n🔍 통합 검색\n🤝 봉사활동\n\n자세한 사용법은 "사용법 안내"를 눌러보세요!', suggests: ['사용법 안내', '앱 소개', '인사이트 안내'] };
  }

  // --- 앱 소개 ---
  if (/앱\s*소개|이\s*앱\s*(뭐|뭔|무엇|어떤|소개)|앱\s*설명|워크플로우\s*(뭐|뭔|소개)|앱이\s*뭐/.test(t)) {
    return { reply: '📱 WorkFlow 앱 소개\n━━━━━━━━━━━━━━\n\n' +
      'WorkFlow는 모바일 중심 스마트 업무관리 시스템이에요.\n팀원들과 함께 업무를 기록하고, 분석하고, 성장하는 플랫폼이죠!\n\n' +
      '🏠 핵심 기능\n' +
      '• 📝 업무일지 — 육하원칙 기반 보고서, AI 자동 작성\n' +
      '• ✅ 할 일 — 우선순위·기한 설정, 완료 추적\n' +
      '• 📅 캘린더 — 업무·일정·할일 통합 달력\n' +
      '• ⏰ 출퇴근 — 출근/퇴근 체크, 지각 관리\n' +
      '• 📊 주간계획 — 요일별 업무 계획 수립\n\n' +
      '🤝 협업 기능\n' +
      ...
      '• 📢 공지사항 — 중요 소식 전달\n' +
      '• 🏢 전국 지국 — 지점 현황 관리\n' +
      '• ✍️ 결재 시스템 — 보고서 승인 워크플로우\n\n' +
      '🧠 AI 기능\n' +
      '• 🤖 AI 비서 — 대화로 업무 처리 (저예요!)\n' +
      '• 📈 인사이트 — 업무 패턴 분석·예측\n' +
      '• 📋 자동 매뉴얼 — 업무 기록 기반 자동 생성\n' +
      '• 🗺️ 지식맵 — 업무 관계도 시각화\n\n' +
      '더 자세히 알고 싶은 부분이 있으면 말씀해주세요!',
      suggests: ['사용법 안내', '인사이트 안내', '오늘 브리핑'] };
  }

  // --- 인사이트 안내 ---
  if (/인사이트\s*안내|인사이트\s*(뭐|뭔|설명|소개|어떤)|분석\s*(기능|안내|소개)|분석\s*뭐/.test(t)) {
    return { reply: '📊 분석 & 인사이트 안내\n━━━━━━━━━━━━━━\n\n업무일지를 쓰면 AI가 자동으로 분석해줘요!\n\n' +
      '1️⃣ AI 인사이트 (추천!)\n' +
      '   업무 데이터를 종합 분석 → 패턴 발견\n   팀 전체 트렌드, 개선점 제안\n   → "AI 인사이트 열어" 또는 더보기 → AI 인사이트\n\n' +
      '2️⃣ 내 업무 분석\n' +
      '   나만의 업무 패턴·카테고리별 비중\n   시간대별 생산성, 강점/약점 분석\n   → "업무 분석" 또는 더보기 → 내 업무 분석\n\n' +
      '3️⃣ 월간 요약\n' +
      '   이번 달 업무·출근·할일·일정 종합 리포트\n   → "이번 달 요약" 또는 더보기 → 월간 요약\n\n' +
      '4️⃣ 주간 보고서\n' +
      '   이번 주 업무 자동 정리, 상사 보고용\n   → "주간 리포트" 또는 더보기 → 주간 보고서\n\n' +
      '5️⃣ 업무 지식맵\n' +
      '   업무 간 관계를 다이어그램으로 시각화\n   → "지식맵" 또는 더보기 → 업무 지식맵\n\n' +
      '6️⃣ 업무 흐름도\n' +
      '   반복 업무의 프로세스를 자동 생성\n   → "흐름도" 또는 더보기 → 업무 흐름도\n\n' +
      '7️⃣ 목표 & 방향\n' +
      '   회의록 기반 조직 목표 분석\n   → "목표" 또는 더보기 → 목표 & 방향\n\n' +
      '💡 일지를 많이 쓸수록 분석이 정확해져요!',
      suggests: ['AI 인사이트 열어', '업무 분석', '이번 달 요약', '사용법 안내'] };
  }
  if (/AI\s*인사이트\s*열어|인사이트\s*열어/.test(t)) {
    return { reply: 'AI 인사이트 페이지로 이동할게요! 📊', action: () => { closeAiChat(); showSmartInsight(); } };
  }
  if (/지식\s*맵/.test(t)) {
    return { reply: '업무 지식맵을 열게요! 🗺️', action: () => { closeAiChat(); showKnowledgeMap(); } };
  }
  if (/흐름도/.test(t)) {
    return { reply: '업무 흐름도를 열게요! 📈', action: () => { closeAiChat(); showWorkflowDiagrams(); } };
  }
  if (/목표|방향/.test(t) && !/할\s*일|다음|계획/.test(t)) {
    return { reply: '목표 & 방향 페이지로 이동할게요! 🎯', action: () => { closeAiChat(); showDirection(); } };
  }
